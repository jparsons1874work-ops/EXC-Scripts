...
The Hub starts the separate `scripts/Tennis_Manual_Watcher_Supervisor.py` controller. It replaces the old two-hour self-reset: at 12:55 UK time and every 30 minutes thereafter (:25 and :55 each hour, including overnight), it stops the watcher and its browser processes, waits 30 seconds after shutdown, then launches a fresh watcher. It also recovers early watcher exits after the same pause. Saved links and persisted alert history survive resets. **Stop watcher** stops the controller as well and cancels pending restarts; select **Start watcher** to enable it again. The Hub remains running during these resets. Launch the supervisor, rather than the underlying watcher script, when running this manually outside the Hub.
