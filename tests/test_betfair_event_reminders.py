    ...
    def test_cli_scan_window_default_and_override(self) -> None:
        for arguments, expected_hours in (([], 72), (["--lookahead-hours", "48"], 48)):
            with self.subTest(arguments=arguments), patch.object(sys, "argv", ["reminders", *arguments]):
                ...
                window = build_scan_window(
                    datetime(2026, 9, 7, 8, 0, tzinfo=UK_TZ),
                    lookahead_hours=args.lookahead_hours,
                )
                self.assertEqual((window.end_utc - window.start_utc).total_seconds(), expected_hours * 3600)

    def test_cycling_retimed_tomorrow_is_selected_to_replace_today_reminder(self) -> None:
        now = datetime(2026, 9, 7, 8, 0, tzinfo=UK_TZ)
        old_market = reminder(
            "Cycling", "28265365",
            datetime(2026, 9, 7, 12, 5, tzinfo=UK_TZ).astimezone(timezone.utc),
            event_name="Vuelta a Espana", market_name="Winner", market_type_code="WINNER",
        )
        old_post_epoch = int(reminder_time(old_market.event_start_utc).timestamp())
